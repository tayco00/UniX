...
export const taskSchema = z.object({
  id: z.string().min(1).max(80).refine((id) => id.trim().length > 0),
...
export const appDataSchema = z.object({
  ...
  tasks: z.array(taskSchema).max(5000).refine((tasks) => new Set(tasks.map((task) => task.id)).size === tasks.length, "Aufgaben-IDs müssen eindeutig sein"),
  settings: z.object({
